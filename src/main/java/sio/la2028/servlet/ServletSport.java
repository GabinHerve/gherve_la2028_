...
public class ServletSport extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if(url.equals("/la2028/ServletSport/listerSport"))
        {
            ArrayList<Sport> lesSport = DaoSport.getLesSports(cnx);
            request.setAttribute("pLesSports", lesSport);
            //System.out.println("lister eleves - nombres d'élèves récupérés" + lesEleves.size() );
            getServletContext().getRequestDispatcher("/vues/athlete/listerSport.jsp").forward(request, response);
        }

        if(url.equals("/la2028/ServletSport/consulter"))
        {
            String paramId = request.getParameter("idSport");
            System.out.println("DEBUG - Valeur reçue de l'URL : " + paramId);

            int idSport = Integer.parseInt(paramId);

            Sport s = DaoSport.getSportById(cnx, idSport);
            request.setAttribute("pSport", s);

            ArrayList<Athlete> lesAthletes = DaoSport.getLesAthletesBySport(cnx, idSport);
