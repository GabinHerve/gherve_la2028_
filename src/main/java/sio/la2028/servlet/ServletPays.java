...
public class ServletPays extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if(url.equals("/la2028/ServletPays/listerPays"))
        {
            ArrayList<Pays> lesPays = DaoPays.getLesPays(cnx);
            request.setAttribute("pLesPays", lesPays);
            //System.out.println("lister eleves - nombres d'élèves récupérés" + lesEleves.size() );
            getServletContext().getRequestDispatcher("/vues/athlete/listerPays.jsp").forward(request, response);
        }

        if(url.equals("/la2028/ServletPays/consulter"))
        {
            String paramId = request.getParameter("idPays");
            System.out.println("DEBUG - Valeur reçue de l'URL : " + paramId);

            int idPays = Integer.parseInt(paramId);

            Pays p = DaoPays.getPaysById(cnx, idPays);
            request.setAttribute("pPays", p);

            ArrayList<Athlete> lesAthletes = DaoPays.getLesAthletesByPays(cnx, idPays);
            System.out.println("DEBUG - Nombre d'athlètes trouvés : " + lesAthletes.size());

            request.setAttribute("lesAthletesPays", lesAthletes);

            getServletContext().getRequestDispatcher("/vues/athlete/consulterPays.jsp").forward(request, response);
        }
